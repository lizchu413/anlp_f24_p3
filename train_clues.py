# https://github.com/jsrozner/decrypt/blob/main/seq2seq/train_clues.py
...
class ClueTrainer(T5Trainer):
    ...
    def pre_tokenize_no_len_multi(batch_list: List[Dict]):
        src_text, tgt_text, idxs = [], [], []
        for e in batch_list:
            input = e['input']
            splits = input.split(' ')
    ...
    def coll_fn_no_len_multi(tokenizer: PreTrainedTokenizerFast, batch_list: List[Dict]) -> Dict:
        return default_collate_fn_json(tokenizer, batch_list, pre_tokenize_fn=ClueTrainer.pre_tokenize_no_len_multi)
    ...
    def setup_dataloaders_no_len_multi(self):
        ...
        self.setup_dataloaders_multi(batched_collate_fcns=None,
                                     multitask_collate_fn=self.coll_fn_no_len_multi)
